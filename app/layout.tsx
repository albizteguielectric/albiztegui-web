  ...
  return (
    <html lang="es">
      <body className="bg-gray-50 text-gray-900 antialiased flex flex-col min-h-screen relative">
        ...
        {/* Botón Flotante Múltiple de WhatsApp */}
        <div className="fixed bottom-24 right-6 flex flex-col items-end z-40">
          <div className="relative group flex flex-col items-end">
            
            {/* Opciones de contacto */}
            <div className="absolute bottom-16 right-0 flex flex-col gap-3 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform translate-y-4 group-hover:translate-y-0">
              <a 
                href="https://wa.me/526361109087" 
                target="_blank" 
                rel="noopener noreferrer"
                className="bg-green-500 text-white px-4 py-2 rounded-full shadow-md hover:bg-green-600 hover:scale-105 transition-all whitespace-nowrap text-sm font-bold text-center"
              >
                Jesús Álvarez
              </a>
              <a 
            ...
            {/* Botón Principal Vectorial */}
            <button 
              className="flex items-center justify-center w-14 h-14 bg-[#25D366] text-white rounded-full shadow-lg hover:bg-[#20ba5a] transition-all hover:scale-110 cursor-pointer border-2 border-white p-0"
