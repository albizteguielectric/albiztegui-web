...
// ESTRUCTURA COMPLETA DE 3 NIVELES PARA CAJAS Y REGISTROS
const ESTRUCTURA_CAJAS: Record<string, Record<string, string[]>> = {
  'CAJAS METALICAS': {
    'Cajas Registro': [
      '2X4 y 2X5',
...
// Función auxiliar para normalizar textos sin acentos ni espacios extra
...
export default function CajasRegistrosPage() {
  ...
  const [productos, setProductos] = useState<ProductoCajas[]>([])
  ...
  useEffect(() => {
    ...
    const fetchProductos = async () => {
      ...
      if (error) {
        console.error('Error al cargar productos_cajas:', error.message || error)
        setCargando(false)
        return
      }
  ...
  const abrirImagenModal = (e: React.MouseEvent, item: ProductoCajas) => {
    e.stopPropagation()
  ...
  const subcategoriasDisponibles = categoriaPrincipal !== 'Todas' && ESTRUCTURA_CAJAS[categoriaPrincipal]
    ? Object.keys(ESTRUCTURA_CAJAS[categoriaPrincipal])
    : Array.from(new Set(Object.values(ESTRUCTURA_CAJAS).flatMap(c => Object.keys(c))))

  const subcategoriasEspecialesDisponibles = (categoriaPrincipal !== 'Todas' && subcategoriaActiva !== 'Todas' && ESTRUCTURA_CAJAS[categoriaPrincipal]?.[subcategoriaActiva])
    ? ESTRUCTURA_CAJAS[categoriaPrincipal][subcategoriaActiva]
    : []
        ...
        <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg border border-gray-100 overflow-hidden mb-8 sm:mb-12 grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 p-4 sm:p-10 items-center">
          ...
          <div className="lg:col-span-7 space-y-4 sm:space-y-6">
            ...
            <p className="text-gray-600 leading-relaxed text-xs sm:text-base">
              Disponemos de todas las variantes de cajas para vivienda, comercio e industria. Desde chalupas tradicionales galvanizadas y plásticas hasta cajas a prueba de agua y sistemas de montaje en canaleta o registros de intemperie.
            </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4 border-t border-gray-100 pt-4 sm:pt-6">
              <div className="bg-blue-50/50 p-3 sm:p-4 rounded-xl border border-blue-100">
                <h3 className="font-bold text-blue-900 text-xs sm:text-sm mb-1">✓ Materiales Certificados</h3>
                <p className="text-[11px] sm:text-xs text-gray-600">Lámina galvanizada reforzada, PVC autoextinguible y tapas de protección.</p>
              </div>
              <div className="bg-orange-50/50 p-3 sm:p-4 rounded-xl border border-orange-100">
                <h3 className="font-bold text-blue-900 text-xs sm:text-sm mb-1">✓ Compatibilidad Total</h3>
                <p className="text-[11px] sm:text-xs text-gray-600">Nocouts con medidas estándar para tuberías y conectores usuales de instalación.</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-12 sm:mb-16">
          <div className="text-center max-w-2xl mx-auto mb-6 sm:mb-8">
            ...
            <h2 className="text-xl sm:text-4xl font-extrabold text-blue-900 mt-2 sm:mt-3">
              Catálogo de Cajas, Registros y Tapas
            </h2>
          ...
          <div className="relative mb-6">
            <div className="flex overflow-x-auto gap-2 pb-3 pt-1 px-1 sm:flex-wrap sm:justify-center sm:overflow-visible scroll-smooth snap-x snap-mandatory">
              ...
              {Object.keys(ESTRUCTURA_CAJAS).map((catKey) => (
                <button
                  key={catKey}
          ...
          {cargando ? (
            <div className="text-center py-16">
              <p className="text-lg font-bold text-blue-900 animate-pulse">Cargando cajas y registros...</p>
            </div>
